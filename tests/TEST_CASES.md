...
- **Run:** `cd tests && ATHENA_DB_BUCKET=<your-results-bucket> pytest -v` (runs every test
  in both files; resource names come from env vars defaulting to
  `terraform.tfvars.example`; the EKS cases are skipped unless `EKS_CLUSTER_NAME` is set)
...
| Test name | When | Then | Details / Result |
|---|---|---|---|
| `test_lambda_runs_inside_the_vpc` | the Lambda config is inspected | it is attached to VPC subnets and a security group | `lambda:GetFunctionConfiguration`; asserts `SubnetIds`/`SecurityGroupIds` non-empty. Result: runs privately inside the VPC, not the public internet. |
